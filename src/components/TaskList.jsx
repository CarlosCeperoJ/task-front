...
const TaskList = ({ tasks, toggleTaskCompletion, deleteTask }) => {
  return (
    <div className="space-y-4">
      {tasks.map(task => (
        <div key={task._id} className={`bg-white p-4 rounded-lg shadow-lg ${task.completed ? 'bg-green-100' : 'bg-red-100'}`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div className="flex-1 mb-4 md:mb-0">
              <h2 className={`text-xl font-bold text-gray-800 break-words ${task.completed ? 'line-through' : ''}`}>
